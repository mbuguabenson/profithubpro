...
import { LegacyClose1pxIcon, LegacyLogout1pxIcon } from '@deriv/quill-icons/Legacy';
...
const InfoPanel = observer(() => {
    ...
    const renderInfo = () => (
        <div className='db-info-panel'>
            ...
            {client.is_logged_in && (
                <button type='button' className='db-info-panel__logout-button' onClick={oAuthLogout}>
                    <div>
                        <Text color='prominent' lineHeight='xl' size={isDesktop ? 's' : 'xs'} weight='bold'>
                            {localize('Logout')}
                        </Text>
                        <Text color='less-prominent' lineHeight='l' size='xs'>
                            {localize('Sign out of your account and return to the homepage.')}
                        </Text>
                    </div>
                    <LegacyLogout1pxIcon height='18px' width='18px' fill='var(--text-prominent)' />
                </button>
            )}
        </div>
    );
