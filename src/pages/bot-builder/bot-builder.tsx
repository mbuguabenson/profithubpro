...
import DeepScanModal from '@/components/deep-scan-modal';
...
const BotBuilder = observer(() => {
  ...
  // Deep Scan modal state and handlers
  const [isDeepScanOpen, setIsDeepScanOpen] = useState(false);
  const handleOpenDeepScan = () => setIsDeepScanOpen(true);
  const handleCloseDeepScan = () => setIsDeepScanOpen(false);
    const { onMount, onUnmount } = app;
    const el_ref = React.useRef<HTMLInputElement | null>(null);

    const handleLoadSignal = (signal: any, settings: any) => {
    // Store signal and settings for Smart Trader
    ...
    dashboard.setActiveTab(DBOT_TABS.SMART_TRADER);
  };
    ...
    return (
        <>
            <div className={classNames('bot-builder', {
            ...
            <div id='scratch_div' ref={el_ref}>
                    <WorkspaceWrapper onOpenScanner={handleOpenDeepScan} />
                </div>
            </div>
        ...
        <DeepScanModal isOpen={isDeepScanOpen} onClose={handleCloseDeepScan} onLoadSignal={handleLoadSignal} />
        </>
    );
});
