...
import {
    ...
    transformBuyRequest,
    transformProposalOpenContractRequest,
...
export class DigitTradeEngine {
    ...
    executeTrade = async (
        ...
        try {
            ...
            // Transform to new API format (removes loginid, barrier_range, product_type, date_start, etc.)
            const transformedProposalData = transformProposalRequest(proposal_data);

            const proposal = (await api.send(transformedProposalData)) as {
            ...
            // Transform to new API format (removes loginid)
            const transformedBuyRequest = transformBuyRequest(buy_request);

            this.addLog(`💰 Buying: id=${transformedBuyRequest.buy} price=${transformedBuyRequest.price}`, 'journal');

            const buy = (await api.send(transformedBuyRequest)) as {
                error?: { message: string; code: string };
    ...
    private monitorTrade = (contract_id: string, config: TTradeConfig) => {
        const check = setInterval(async () => {
            try {
                // Transform to new API format (removes loginid)
                const pocRequest = transformProposalOpenContractRequest({ proposal_open_contract: 1, contract_id });
                const data = (await api_base.api?.send(pocRequest)) as {
